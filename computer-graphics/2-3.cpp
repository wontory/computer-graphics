...
void display() {
    ...
    glViewport(640, 0, 640, 490);                                       // Window의 좌측 하단을 원점으로 (640, 0) 위치에 (640, 490) 크기의 Viewport 생성 -> 우측 하단 1/4 영역에 표시
    ...
    glTranslatef(4.0, 0.25, -1.3);                                      // x, y, z축으로의 이동 값 정의, (4, 0.25, -1.3) 위치로 이동
    ...
    // Teapot
    glPushMatrix();                                                     // 행렬 스택에 현재 좌표축 저장
    glTranslatef(1.5, 0.1, -3.3);                                       // x, y, z축으로의 이동 값 정의, (1.5, 0.1, -3.3) 위치로 이동
